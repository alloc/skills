...
- Package: `react-aria`
- Import: `import {useTableSelectionCheckbox} from 'react-aria'`
...
```ts
useTableSelectionCheckbox<T>(props: AriaTableSelectionCheckboxProps, state: TableState<T>): TableSelectionCheckboxAria
```

## Use For

Use `useTableSelectionCheckbox` for the table selection checkbox part of the `useTable` pattern. The upstream page describes the broader pattern as: Provides the behavior and accessibility implementation for a table component. A table displays data in rows and columns and enables a user to navigate its contents via directional navigation keys, and optionally supports row selection and sorting.
