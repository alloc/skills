...
- Package: `react-aria`
- Import: `import {useMenuTrigger} from 'react-aria'`
...
```ts
useMenuTrigger<T>(props: AriaMenuTriggerProps, state: MenuTriggerState, ref: RefObject<Element | null>): MenuTriggerAria<T>
```

## Use For

Use `useMenuTrigger` for the menu trigger part of the `useMenu` pattern. The upstream page describes the broader pattern as: Provides the behavior and accessibility implementation for a menu component. A menu displays a list of actions or options that a user can choose.
