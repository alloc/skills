...
- Package: `react-aria`
- Import: `import {useCalendarGrid} from 'react-aria'`
...
```ts
useCalendarGrid(props: AriaCalendarGridProps, state: CalendarState<CalendarSelectionMode> | RangeCalendarState): CalendarGridAria
```

## Use For

Use `useCalendarGrid` for the calendar grid part of the `useCalendar` pattern. The upstream page describes the broader pattern as: Provides the behavior and accessibility implementation for a calendar component. A calendar displays one or more date grids and allows users to select a single date.
