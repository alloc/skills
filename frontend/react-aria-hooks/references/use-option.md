...
- Package: `react-aria`
- Import: `import {useOption} from 'react-aria'`
...
```ts
useOption<T>(props: AriaOptionProps, state: ListState<T>, ref: RefObject<FocusableElement | null>): OptionAria
```

## Use For

Use `useOption` for the option part of the `useListBox` pattern. The upstream page describes the broader pattern as: Provides the behavior and accessibility implementation for a listbox component. A listbox displays a list of options and allows a user to select one or more of them.
