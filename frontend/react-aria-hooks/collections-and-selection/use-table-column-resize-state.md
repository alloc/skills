...
- Import: `import {useTableColumnResizeState} from 'react-stately'`
- Install: `yarn add react-stately`
- Source version: 3.48.0
- Documented on upstream page: `useTableState`
...
Use `useTableColumnResizeState` for the table column resize state part of the `useTableState` pattern. The upstream page describes the broader pattern as: Provides state management for a table component. Handles building a collection of columns and rows from props. In addition, it tracks row selection and manages sort order changes.
...
- [useTableState](./use-table-state.md)
